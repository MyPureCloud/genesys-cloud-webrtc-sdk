...
export default function useAuth() {
  ...
  async function authenticateFromUrlToken() {
    ...
    window.localStorage.setItem(`${persistentName}_auth_data`, JSON.stringify({ accessToken: token }));
    ...
    console.warn('time to do it big');
  ...
  function authenticateImplicitly (environment: any) {
    ...
    client.loginImplicitGrant(environmentData.clientId, window.location.href)
      .then(() => {
        const authInfo = JSON.parse((window as any).localStorage.getItem(`${persistentName}_auth_data`));
        ...
        (window as any).conversationsAPI = new platformClient.ConversationsApi();
        console.warn('do it big implicitly');
        // TODO: Create a custom hook for initiating the SDK.
