...
describe('endSession', () => {
  it('should end session and stop all tracks', async () => {
    const baseSpy = jest.spyOn(BaseSessionHandler.prototype, 'endSession').mockResolvedValue();
...
describe('setVideoMute', () => {
  ...
  beforeEach(() => {
    jest.spyOn(handler, 'removeMediaFromSession').mockResolvedValue();
...
describe('pinParticipantVideo', () => {
  ...
  it('should unpin participant', async () => {
    ...
    const eventSpy = jest.fn();
    session.on('pinnedParticipant', eventSpy);
    await handler.pinParticipantVideo(session, undefined);
...
describe('handleDataChannelMessage', () => {
  it('should call handleMemberStatusMessage', () => {
    ...
    const message: JsonRpcMessage<Pick<GenesysDataChannelMessageParams, 'member.notify.status'>> = {
      jsonrpc: 'v2',
